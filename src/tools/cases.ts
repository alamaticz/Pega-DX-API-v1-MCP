 ...
 *  7. get_case_page      — GET  /cases/{ID}/pages/{pageID}
 *  8. get_case_action    — GET  /cases/{ID}/actions/{actionID}
 *  9. refresh_case_action — PUT /cases/{ID}/actions/{actionID}/refresh
 */
...
export const caseToolDefinitions: Tool[] = [
  ...
  },
  {
    name: "get_case_page",
    description:
      "Get the details of a specific embedded page within a case [DX API]. Returns page data and metadata.",
    inputSchema: {
      type: "object",
      properties: {
        caseID: {
        ...
        pageID: {
          type: "string",
          description: "Name of the page to retrieve, e.g. 'pyWorkPage'.",
        },
      },
      required: ["caseID", "pageID"],
    },
  },
  {
    name: "get_case_action",
    description:
      "Get form metadata and field details for a specific action as applied to a case [DX API]. Returns view structure and layout information.",
    inputSchema: {
      type: "object",
      properties: {
        caseID: {
        ...
        actionID: {
          type: "string",
          description:
            "Flow action ID applied to the case, e.g. 'pyUpdateCaseDetails'. IDs typically have no spaces.",
        },
      },
  ...
  },
  {
    ...
    description:
      "Refresh action details for a case after a field change [DX API]. Executes configured Data Transforms and returns updated field values for dynamic form behaviour.",
    inputSchema: {
      type: "object",
      properties: {
        caseID: {
...
export async function handleCaseTool(
  ...
  switch (toolName) {
    ...
    case "get_case_page": {
      const parsed = z
        .object({ caseID: z.string(), pageID: z.string() })
        .parse(args);
      const result = await pegaGet(
        `/cases/${encodeURIComponent(parsed.caseID)}/pages/${encodeURIComponent(parsed.pageID)}`
      );
      return JSON.stringify(result, null, 2);
    }
    ...
    case "get_case_action": {
      const parsed = z
        .object({ caseID: z.string(), actionID: z.string() })
        .parse(args);
      const result = await pegaGet(
        `/cases/${encodeURIComponent(parsed.caseID)}/actions/${encodeURIComponent(parsed.actionID)}`
      );
      return JSON.stringify(result, null, 2);
    }

    case "refresh_case_action": {
      const parsed = z
        .object({
          caseID: z.string(),
          actionID: z.string(),
      ...
      const body: Record<string, unknown> = {};
      if (parsed.content) body.content = parsed.content;
      if (parsed.pageInstructions) body.pageInstructions = parsed.pageInstructions;

      const result = await pegaPut(
        `/cases/${encodeURIComponent(parsed.caseID)}/actions/${encodeURIComponent(parsed.actionID)}/refresh`,
        body
      );
      return JSON.stringify(result, null, 2);
    }
